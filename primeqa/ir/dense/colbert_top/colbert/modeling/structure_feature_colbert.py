...
from primeqa.ir.dense.colbert_top.colbert.modeling.structure_feature_base_colbert import StructureFeatureBaseColBERT
...
class StructureFeatureColBERT(StructureFeatureBaseColBERT):
    """
    ...
    def __init__(self, name=None, colbert_config=None):
        ...
        StructureFeatureColBERT.try_load_torch_extensions(self.use_gpu)
    ...
    def forward(self, Q, D, Q_structure_features, D_structure_features):
        Q = self.query(*Q)
        ...
        for q_feature, d_feature in zip(Q_structure_features, D_structure_features):
            q = self.query(*q_feature)
            d, d_mask = self.doc(*d_feature, keep_dims='return_mask')
            q_duplicated = q.repeat_interleave(self.colbert_config.nway, dim=0).contiguous()
            scores = self.score(q_duplicated, d, d_mask)
            all_scores.append(scores)
...
def colbert_score_packed(Q, D_packed, D_lengths, config=ColBERTConfig()):
    ...
    if use_gpu or config.interaction == "flipr":
        ...
        return StructureFeatureColBERT.segmented_maxsim(scores, D_lengths)
